...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900 border-b border-slate-800 text-slate-100 sticky top-0 z-40 shadow-lg">
      {/* Top Bar: Title, Chart+Bot Button, Running Live Ticker, Paper Account & Settings */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-4">
        {/* Brand & Title + Chart & Bot Tab (same row) */}
        <div className="flex items-center gap-3 shrink-0">
          ...
          {/* ชาร์ต & ควบคุมบอท — อยู่ระนาบเดียวกับ Logo */}
          <div className="hidden sm:flex items-center ml-2 border-l border-slate-700 pl-3">
            <button
      ...
      <div className="bg-slate-950/80 border-t border-slate-800/80 px-3 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex space-x-1 sm:space-x-2 overflow-x-auto py-2 scrollbar-none touch-pan-x overscroll-x-contain">
          {/* ชาร์ต & ควบคุมบอท: แสดงเฉพาะมือถือ (desktop อยู่ใน Top Bar แล้ว) */}
          <button
            onClick={() => setActiveTab('chart')}
            className={`sm:hidden flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-medium transition whitespace-nowrap ${
              activeTab === 'chart'
